...
mDatei.add_command(label="Offnen")
...
farbe.set("#FAAC58")
mAnsicht.add_radiobutton(label="Braun", variable=farbe, value="#FAAC58",
                         underline=0, command=farbwechsel)
mAnsicht.add_radiobutton(label="Grun", variable=farbe, value="#ACFA58",
                         underline=0, command=farbwechsel)
mAnsicht.add_separator()

rand = tkinter.StringVar()
rand.set("flat")
mAnsicht.add_checkbutton(label="Rand", variable=rand, onvalue="solid", offvalue="flat",
                         underline=0, command=randwechsel)
