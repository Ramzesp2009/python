...
class Fenster(wi.QWidget):
    def __init__(self):
        ...
        buEinfuegen = wi.QPushButton("Einfugen")
        buEinfuegen.clicked.connect(self.einfugen)
        gr.addWidget(buEinfuegen, 1, 0)

        buAendern = wi.QPushButton("Aendern")
        buAendern.clicked.connect(self.aendern)
        gr.addWidget(buAendern, 2, 0)

        buLoeschen = wi.QPushButton("Loeschen")
        ...
        # Datenbankdatei prufen, ggf. erzeugen
        ...
        # Fenster gestalten und anzeien
        self.setWindowTitle("Datenbank, Verwoltung")
        self.setLayout(gr)
        self.show()

    # Hilfsmethode: Anzeige eined SQLite-Fehlers
    def fehler(self, e):
    ...
    def auswahl(self):
        ...
        if i == -1:
            self.meldung("Kein Datensatz ausgewahlt")
            return
        try:
            connection = sqlite3.connect("firma.db")
            cursor = connection.cursor()
            sql = "SELECT * FROM personen WHERE personalnymmer = " \
                + str (self.pnummer[i])
            cursor.execute(sql)
            for dsatz in cursor:
    ...
    def einfugen(self):
        ...
        try:
            connection = sqlite3.connect("firma.db")
            cursor = connection.cursor()
            sql = "INSERT INTO personen VELUES(?, ?, ?, ?, ?)"
    ...
    def aendern(self):
        i = self.liAnzeige.currentRow()
        if i == -1:
            self.meldung("Kein Datensatx ausgewahlt")
            return
        try:
            connection = sqlite3.connect("firma.db")
            cursor = connection.cursor()
